...
(function (factory) {
    ...
    $.extend(DateTime.prototype, {
         ...
         * @param  {boolean} [write=true] Flag to indicate if the formatted
         *     value should be written into the input element
         */
        val: function (set, write) {
            ...
            if (set instanceof Date) {
                ...
                // luxon uses different method names so need to be able to call
                // them
                if (dateLib && dateLib == window.luxon) {
                    ...
                    // Use moment, dayjs or luxon if possible (even for ISO8601
                    // strings, since it will correctly handle 0000-00-00 and
                    // the like)
                    var m = dateLib.utc(set, this.c.format, this.c.locale, this.c.strict);
                    this.s.d = m.isValid() ? m.toDate() : null;
                } else {
                    // Else must be using ISO8601 without a date library
                    // (constructor would have thrown an error otherwise)
            ...
            // Set the day of the month to be 1 so changing between months
            // doesn't run into issues when going from day 31 to 28 (for
            // example)
        ...
        _constructor: function () {
            ...
            // When attached to a hidden input, we always show the input
            // picker, and do so inline
            ...
            // Trigger the display of the widget when clicking or focusing on
            // the input element
            this.dom.input
            ...
            this.dom.container
                .on('change', 'select', function () {
                    ...
                    if (select.hasClass(classPrefix + '-month')) {
                        ...
                        // Hours - need to take account of AM/PM input if
                        // present
                ...
                .on('click', function (e) {
                    ...
                    if (nodeName === 'button') {
                        ...
                        if (parent.hasClass(classPrefix + '-iconLeft')) {
                            ...
                            // Can't be certain that the current day will exist
                            // in the new month, and likewise don't know that
                            // the new day will exist in the old month, But 1
                            // always does, so we can change the month without
                            // worry of a recalculation being done
                            ...
                            // Don't hide if there is a time picker, since we
                            // want to be able to select a time as well.
                            if (!that.s.parts.time) {
                                // This is annoying but IE has some kind of
                                // async behaviour with focus and the focus
                                // from the above write would occur after this
                                // hide - resulting in the calendar opening
                                // immediately
                                setTimeout(function () {
                                    that._hide();
                                }, 10);
                        ...
                        // Click anywhere else in the widget - return focus to
                        // the input element
                        that.dom.input.focus();
                    }
                });
        ...
        /**
         * Compare the date part only of two dates - this is made super easy by
        ...
        /**
         * When changing month, take account of the fact that some months don't
         * have the same number of days. For example going from January to
         * February you can have the 31st of Jan selected and just add a month
         * since the date would still be 31, and thus drop you into March.
         *
        ...
        /**
         * Create a new date object which has the UTC values set to the local
         * time. This allows the local time to be used directly for the library
         * which always bases its calculations and display on UTC.
         *
        ...
        /**
         * Generate the HTML for a single day in the calendar - this is
         * basically and HTML cell with a button that has data attributes so we
         * know what was clicked on (if it is clicked on) and a bunch of
         * classes for styling.
         *
         ...
         *
         * @param  {string} selector Class name unique to the select element to
         *     use
        ...
        /**
         * Set an option and update the option's span pair (since the select
         * element has opacity 0 for styling)
         *
         * @param  {string} selector Class name unique to the select element to
         *     use
        ...
        _position: function () {
            ...
            if (calWidth + offset.left > $(window).width()) {
                ...
                // Account for elements which are inside a position absolute
                // element
                if (this.c.attachTo === 'input') {
        ...
        /**
         * Set the month and year for the calendar based on the current display
         * date
         *
         * @private
         */
        ...
        _setTime: function () {
            ...
            // luxon uses different method names so need to be able to call
            // them. This happens a few time later in this method too
            var luxDT = null
            if (dateLib && dateLib == window.luxon) {
        ...
        _show: function () {
            ...
            // On tab focus will move to a different field (no keyboard
            // navigation in the date picker - this might need to be changed).
        ...
        /**
         * Write the formatted string to the input element this control is
         * attached to
         *
         * @private
         */
        _writeOutput: function (focus) {
            ...
            // Use moment, dayjs or luxon if possible - otherwise it must be
            // ISO8601 (or the constructor would have thrown an error) luxon
            // uses different method names so need to be able to call them.
            if (date) {
