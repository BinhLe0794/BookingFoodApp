...
(function () {
    ...
    var FixedColumns = /** @class */ (function () {
         ...
         *
         * @param newVal Optional. If present this will be the new value for
         *     the number of left fixed columns
         ...
         *
         * @param newVal Optional. If present this will be the new value for
         *     the number of right fixed columns
        ...
        /**
         * Iterates over the columns, fixing the appropriate ones to the left
         * and right
         */
        FixedColumns.prototype._addStyles = function () {
            ...
            if (footer !== null) {
                ...
                // Only attempt to retrieve the parentDiv if it has not been
                // retrieved already
                if (parentDiv === null) {
                    parentDiv = $(footer.closest('div.dataTables_scroll')).css('position', 'relative');
                }
            }
            // Get the number of columns in the table - this is used often so
            // better to only make 1 api call
            var numCols = this.s.dt.columns().data().toArray().length;
            // Tracker for the number of pixels should be left to the left of
            // the table
            var distLeft = 0;
            // Sometimes the headers have slightly different widths so need to
            // track them individually
            var headLeft = 0;
            // Get all of the row elements in the table
            var rows = $(this.s.dt.table().node()).children('tbody').children('tr');
            var invisibles = 0;
            // When working from right to left we need to know how many are
            // invisible before a point, without including those that are
            // invisible after
            ...
            for (var i = 0; i < numCols; i++) {
                ...
                // If i is less than the value of left then this column should
                // be fixed left
                if (i - invisibles < this.c.left) {
                    $(this.s.dt.table().node()).addClass(this.classes.tableFixedLeft);
                    parentDiv.addClass(this.classes.tableFixedLeft);
                    // Add the width of the previous node - only if we are on
                    // atleast the second column
                    if (i - invisibles > 0) {
                        var prevIdx = i;
                        // Simply using the number of hidden columns doesn't
                        // work here, if the first is hidden then this would be
                        // thrown off
                    ...
                    // Iteriate through all of the rows, making sure they
                    // aren't currently trying to fix left
                    for (var _a = 0, rows_2 = rows; _a < rows_2.length; _a++) {
                        var row = rows_2[_a];
                        var cell = $($(row).children()[i - invisibles]);
                        // If the cell is trying to fix to the left, remove the
                        // class and the css
            ...
            // If there is a header with the index class and reading rtl then
            // add left top blocker
            ...
            // If there is a footer with the index class and reading rtl then
            // add left bottom blocker
            ...
            for (var i = numCols - 1; i >= 0; i--) {
                ...
                if (prev === undefined) {
                    // If it wasn't set then it was the last column so just use
                    // the final value
                    prev = invisibles;
                }
                if (i + rightInvisibles >= numCols - this.c.right) {
                    ...
                    // Add the widht of the previous node, only if we are on
                    // atleast the second column
                    ...
                    // Iterate over all of the rows, fixing the cell to the
                    // right
                    ...
                    // Iteriate through all of the rows, making sure they
                    // aren't currently trying to fix right
                    for (var _c = 0, rows_4 = rows; _c < rows_4.length; _c++) {
                        var row = rows_4[_c];
                        var cell = $($(row).children()[i - prev]);
                        // If the cell is trying to fix to the right, remove
                        // the class and the css
        ...
        /**
         * Gets the correct CSS for the cell, header or footer based on options
         * provided
         *
         * @param header Whether this cell is a header or a footer
         * @param dist The distance that the cell should be moved away from the
         *     edge
        ...
        FixedColumns.prototype._setKeyTableListener = function () {
            ...
            this.s.dt.on('key-focus', function (e, dt, cell) {
                ...
                if (_this.c.left > 0) {
                    // Get the rightmost left fixed column header, it's
                    // position and it's width
                    var rightMost = $(_this.s.dt.column(_this.c.left - 1).header());
                    var rightMostPos = rightMost.offset();
                    var rightMostWidth = rightMost.outerWidth();
                    // If the current highlighted cell is left of the rightmost
                    // cell on the screen
                ...
                if (_this.c.right > 0) {
                    // Get the number of columns and the width of the cell as
                    // doing right side calc
                    var numCols = _this.s.dt.columns().data().toArray().length;
                    var cellWidth = $(cell.node()).outerWidth();
                    // Get the leftmost right fixed column header and it's
                    // position
                    var leftMost = $(_this.s.dt.column(numCols - _this.c.right).header());
                    var leftMostPos = leftMost.offset();
                    // If the current highlighted cell is right of the leftmost
                    // cell on the screen
            ...
            // Whenever a draw occurs there is potential for the data to have
            // changed and therefore also the column widths Therefore it is
            // necessary to recalculate the values for the fixed columns
