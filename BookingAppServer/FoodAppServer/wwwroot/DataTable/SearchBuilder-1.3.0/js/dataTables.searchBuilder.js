...
(function () {
    ...
    /**
     * The Criteria class is used within SearchBuilder to represent a search
     * criteria
     */
    var Criteria = /** @class */ (function () {
        function Criteria(table, opts, topGroup, index, depth) {
            ...
            // Get options from user and any extra conditions/column types
            // defined by plug-ins
            ...
            // If the greyscale option is selected then add the class to add
            // the grey colour to SearchBuilder
            if (this.c.greyscale) {
            ...
            // For responsive design, adjust the criterias properties on the
            // following events
        ...
        Criteria.prototype.updateArrows = function (hasSiblings, redraw) {
            ...
            // Empty the container and append all of the elements in the
            // correct order
            ...
            this.setListeners();
            // Trigger the inserted events for the value elements as they are
            // inserted
            ...
            // If the depthLimit of the query has been hit then don't add the
            // right button
            ...
            if (redraw) {
                // A different combination of arrows and selectors may lead to
                // a need for responsive to be triggered
        ...
        /**
         * Passes in the data for the row and compares it against this single
         * criteria
         *
        ...
        Criteria.prototype.search = function (rowData, rowIdx) {
            ...
            if (this.s.condition !== undefined && condition !== undefined) {
                ...
                // This check is in place for if a custom decimal character is
                // in place
                ...
                // If orthogonal data is in place we need to get it's values
                // for searching
                ...
                // when the value set is null, so we shall assume the two are
                // equal
                if (filter === null) {
                    filter = '';
                }
        ...
        Criteria.prototype.getDetails = function (deFormatDates) {
            ...
            // This check is in place for if a custom decimal character is in
            // place
            if (this.s.type !== null &&
        ...
        /**
         * Populates the criteria data, condition and value(s) as far as has
         * been selected
         */
        ...
        Criteria.prototype.rebuild = function (loadedCriteria) {
            // Check to see if the previously selected data exists, if so
            // select it
            var foundData = false;
            var dataIdx;
            this._populateData();
            // If a data selection has previously been made attempt to find and
            // select it
            ...
            // If the data has been found and selected then the condition can
            // be populated and searched
            if (foundData) {
                ...
                // Check to see if the previously selected condition exists, if
                // so select it
                ...
                // If the condition has been found and selected then the value
                // can be populated and searched
                if (this.s.condition !== undefined) {
        ...
        Criteria.prototype.setListeners = function () {
            ...
            this.dom.data
                ...
                .on('change.dtsb', function () {
                    ...
                    // Need to go over every option to identify the correct
                    // selection
                    ...
                    for (var i = 0; i < options.length; i++) {
                        ...
                        if (option.val() === _this.dom.data.val()) {
                            ...
                            // When the data is changed, the values in
                            // condition and value may also change so need to
                            // renew them
                            _this._clearCondition();
                            _this._clearValue();
                            _this._populateCondition();
                            // If this criteria was previously active in the
                            // search then remove it from the search and
                ...
                .on('change.dtsb', function () {
                    ...
                    for (var i = 0; i < options.length; i++) {
                        ...
                        if (option.val() === _this.dom.condition.val()) {
                            ...
                            // Find the condition that has been selected and
                            // store it internally
                            ...
                            // When the condition is changed, the value
                            // selector may switch between a select element and
                            // an input element
                            _this._clearValue();
                            _this._populateValue();
                            for (var _b = 0, _c = _this.dom.value; _b < _c.length; _b++) {
                                var val = _c[_b];
                                // If this criteria was previously active in
                                // the search then remove it from the search
        ...
        Criteria.prototype._clearValue = function () {
            if (this.s.condition !== undefined) {
                ...
                // Call the init function to get the value elements for this
                // condition
        ...
        Criteria.prototype._populateCondition = function () {
            ...
            // If there are no conditions stored then we need to get them from
            ...
            if (conditionsLength === 0) {
                ...
                // If the column type is still unknown, call a draw to try
                // reading it again
                ...
                // Select which conditions are going to be used based on the
                // column type
                ...
                for (var _i = 0, _a = Object.keys(conditionObj); _i < _a.length; _i++) {
                    ...
                    if (conditionObj[condition] !== null) {
                        // Serverside processing does not supply the options
                        // for the select elements Instead input elements need
                        // to be used for these instead
        ...
        Criteria.prototype._populateData = function () {
            ...
            // If there are no datas stored then we need to get them from the
            // table
            if (this.s.dataPoints.length === 0) {
                this.s.dt.columns().every(function (index) {
                    // Need to check that the column can be filtered on before
                    // adding it
         ...
         *
         * @param loadedCriteria optional, used to reload criteria from
         *     predefined filters
         */
        Criteria.prototype._populateValue = function (loadedCriteria) {
            ...
            // Find the column with the title matching the data for the
            // criteria and take note of the index
        ...
        /**
         * Provides throttling capabilities to SearchBuilder without having to
         * use dt's _fnThrottle function This is because that function is not
         * quite suitable for our needs as it runs initially rather than
         * waiting
         *
        ...
        Criteria.initSelect = function (that, fn, preDefined, array) {
            ...
            // Declare select element to be used with all of the default
            ...
            for (var _i = 0, indexArray_1 = indexArray; _i < indexArray_1.length; _i++) {
                ...
                // If we are dealing with an array type, either make sure we
                // are working with arrays, or sort them
                ...
                var addOption = function (filt, text) {
                    // Add text and value, stripping out any html if that is
                    // the column type
                    var opt = $$2('<option>', {
                        ...
                        // Have to add the text this way so that special html
                        // characters are not escaped - &amp; etc.
                    ...
                    if (added.indexOf(val) === -1) {
                        ...
                        // If this value was previously selected as indicated
                        // by preDefined, then select it again
                ...
                // If this is to add the individual values within the array we
         ...
         *
         * This exists because there needs to be different select functionality
         * for contains/without and equals/not
         */
        ...
        Criteria.initInput = function (that, fn, preDefined) {
            ...
            // This is add responsive functionality to the logic button without
            // redrawing everything else
        ...
        Criteria.init2Date = function (that, fn, preDefined) {
            ...
            var searchDelay = that.s.dt.settings()[0].searchDelay;
            // Declare all of the date elements that are required using
        ...
        Criteria.inputValueSelect = function (el) {
            var values = [];
            // Go through the select elements and push each selected option to
            // the return array
            for (var _i = 0, el_3 = el; _i < el_3.length; _i++) {
                var element = el_3[_i];
                if (element.is('select')) {
                    var val = element.children('option:selected').val();
                    // If the type of the option is an array we need to split
                    // it up and sort it
        ...
        Criteria.inputValueInput = function (el) {
            var values = [];
            // Go through the input elements and push each value to the return
            // array
        ...
        /**
         * Function that is run on each element as a call back when a search
         * should be triggered
         */
        Criteria.updateListener = function (that, el) {
            // When the value is changed the criteria is now complete so can be
            // included in searches Get the condition from the map based on the
            // key that has been selected for the condition
            ...
            // Take note of the cursor position so that we can refocus there
            // later
            var idx = null;
            var cursorPos = null;
        ...
        // Has to be in this order so that they are displayed correctly in
        // select elements Also have to disable member ordering for this as the
        // private methods used are not yet declared otherwise
        ...
        // The order of the conditions will make eslint sad :(
        // Also have to disable member ordering for this as the private methods
        // used are not yet declared otherwise eslint-disable-next-line
        ...
        // eslint will be sad because we have to disable member ordering for
        // this as the private static properties used are not yet declared
        // otherwise eslint-disable-next-line
        // @typescript-eslint/member-ordering
        Criteria.defaults = {
            columns: true,
            conditions: {
    ...
    /**
     * The Group class is used within SearchBuilder to represent a group of
     * criteria
     */
    var Group = /** @class */ (function () {
        function Group(table, opts, topGroup, index, isChild, depth) {
            ...
            // A reference to the top level group is maintained throughout any
            // subgroups and criteria that may be created
        ...
        /**
         * Destroys the groups buttons, clears the internal criteria and
        ...
        Group.prototype.rebuild = function (loadedDetails) {
            ...
            // For all of the criteria children, update the arrows incase they
            // require changing and set the listeners
        ...
        Group.prototype.redrawContents = function () {
            ...
            this.dom.container
                ...
                .append(this.dom.add);
            // Sort the criteria by index so that they appear in the correct
            // order
        ...
        /**
         * Search method, checking the row data against the criteria in the
         * group
         *
        ...
        Group.prototype.setupLogic = function () {
            ...
            // If there are no criteria in the group then keep the logic
            // removed and return
        ...
        Group.prototype.setListeners = function () {
            ...
            this.dom.add.on('click.dtsb', function () {
                // If this is the parent group then the logic button has not
                // been added yet
                if (!_this.s.isChild) {
        ...
        Group.prototype.addCriteria = function (crit, redraw) {
            ...
            // If a Criteria has been passed in then set the values to continue
            // that
            if (crit !== null) {
                criteria.c = crit.c;
                criteria.s = crit.s;
        ...
        /**
         * Gets the count for the number of criteria in this group and any sub
         * groups
         */
        ...
        Group.prototype._andSearch = function (rowData, rowIdx) {
            ...
            // If we get to here then everything has passed, so return true for
            // the group
            return true;
        };
        /**
        ...
        Group.prototype._orSearch = function (rowData, rowIdx) {
            ...
            // This will check to make sure that at least one criteria in the
            // group is complete
        ...
        Group.prototype._removeCriteria = function (criteria, group) {
            ...
            // If removing a criteria and there is only then then just destroy
            // the group
            if (this.s.criteria.length <= 1 && this.s.isChild) {
                ...
                // We want to remove the last element with the desired index,
                // as its replacement will be inserted before it
                if (last !== undefined) {
        ...
        Group.prototype._setCriteriaListeners = function (criteria) {
            ...
            criteria.dom.left
                ...
                .on('click.dtsb', function () {
                    ...
                    // The dropCriteria event mutates the reference to the
                    // index so need to store it
                    var index = _this.s.toDrop.s.index;
                    _this.dom.container.trigger('dtsb-dropCriteria');
                    criteria.s.index = index;
                    _this._removeCriteria(criteria);
                    // By tracking the top level group we can directly trigger
                    // a redraw on it, bubbling is also possible, but that is
                    // slow with deep levelled groups
                    _this.s.topGroup.trigger('dtsb-redrawContents');
                    _this.s.dt.draw();
                    return false;
                });
     ...
     * Allows for complex search queries to be constructed and implemented on a
     * DataTable
     */
    var SearchBuilder = /** @class */ (function () {
        ...
        /**
         * Gets the details required to rebuild the SearchBuilder as it
         * currently is
         */
        ...
        SearchBuilder.prototype._setUp = function (loadState) {
            ...
            // Check that DateTime is included, If not need to check if it
            // could be used eslint-disable-next-line no-extra-parens
            if (!dataTable.DateTime) {
                ...
                // If the types are not yet set then draw to see if they can be
                // retrieved then
                if (types === undefined || types.includes(undefined) || types.includes(null)) {
            ...
            if (loadState) {
                ...
                // If the loaded State is not null rebuild based on it for
                // statesave
        ...
        SearchBuilder.prototype._build = function () {
            ...
            if (!$.fn.dataTable.ext.search.includes(this.s.search)) {
                ...
                // Add SearchBuilder search function to the dataTables search
                // array
    ...
    (function (factory) {
        ...
        $.fn.dataTable.ext.buttons.searchBuilder = {
            action: function (e, dt, node, config) {
                ...
                // Need to redraw the contents to calculate the correct
        ...
        apiRegister('searchBuilder.getDetails()', function (deFormatDates) {
            ...
            // If SearchBuilder has not been initialised on this instance then
            // return
