...
(function () {
    ...
    var SearchPane = /** @class */ (function () {
         ...
         * @returns {object} the pane that has been created, including the
         *     table and the index of the pane
         */
        function SearchPane(paneSettings,
                            opts,
                            idx,
                            layout,
                            panesContainer,
            ...
            this.s.searchFunction = function (settings,
                                              searchData,
                                              dataIndex,
                                              origData) {
            ...
            // Sometimes the top row of the panes containing the search box and
            // ordering buttons appears weird if the width of the panes is
            // lower than expected, this fixes the design. Equally this may
            // occur when the table is resized.
            ...
            // When column-reorder is present and the columns are moved, it is
            // necessary to reassign all of the panes indexes to the new index
            // of the column.
         ...
         * @param filter the value to be filtered on when searchpanes is
         *     implemented
         * @param shown the number of rows in the table that are currently
         *     visible matching this criteria
         * @param total the total number of rows in the table that match this
         *     criteria
         * @param sort the value to be sorted in the pane table
         * @param type the value of which the type is to be derived from
         */
        SearchPane.prototype.addRow = function (display,
                                                filter,
                                                shown,
                                                total,
                                                sort,
        ...
        SearchPane.prototype.adjustTopRow = function () {
            ...
            // If the width is 0 then it is safe to assume that the pane has
            // not yet been displayed. Even if it has, if the width is 0 it
            // won't make a difference if it has the narrow class or not
        ...
        /**
         * In the case of a rebuild there is potential for new data to have
         * been included or removed so all of the rowData must be reset as a
         * precaution.
         */
        ...
        SearchPane.prototype.clearPane = function () {
            // Deselect all rows which are selected and update the table and
            // filter count.
        ...
        /**
         * Strips all of the SearchPanes elements from the document and turns
         * all of the listeners for the buttons off
         */
        SearchPane.prototype.destroy = function () {
            ...
            // If the datatables have been defined for the panes then also
            // destroy these
            if (this.s.dtPane !== undefined) {
         ...
         *
         * @param? last boolean to indicate if this is the last pane a
         *     selection was made in
         * @param? dataIn data to be used in buildPane
         * @param? init Whether this is the initial draw or not
         * @param? maintainSelection Whether the current selections are to be
         *     maintained over rebuild
         */
        SearchPane.prototype.rebuildPane = function (last,
            ...
            this.s.serverSelect = [];
            var prevEl = null;
            // When rebuilding strip all of the HTML Elements out of the
        ...
        /**
         * removes the pane from the page and sets the displayed property to
         * false.
         */
        ...
        /**
         * Sets the cascadeRegen property of the pane. Accessible from above
         * because as SearchPanes.ts deals with the rebuilds.
         *
         * @param val the boolean value that the cascadeRegen property is to be
         *     set to
         */
        ...
        /**
         * This function allows the clearing property to be assigned. This is
         * used when implementing cascadePane. In setting this to true for the
         * clearing of the panes selection on the deselects it forces the pane
         * to repopulate from the entire dataset not just the displayed values.
         *
         * @param val the boolean value which the clearing property is to be
         *     assigned
         */
        ...
        SearchPane.prototype.updateTable = function () {
            ...
            this._searchExtras();
            // If either of the options that effect how the panes are displayed
            // are selected then update the Panes
        ...
        SearchPane.prototype._setListeners = function () {
            ...
            var t0;
            // When an item is selected on the pane, add these to the array
            // which holds selected items. Custom search will perform.
            ...
            // When an item is deselected on the pane, re add the currently
            // selected items to the array which holds selected items. Custom
            ...
            // If we attempty to turn off this event then it will ruin
            // behaviour in other panes so need to make sure that it is only
            // done once
            if (this.s.firstSet) {
                this.s.firstSet = false;
                // When saving the state store all of the selected rows for
                // preselection next time around
                this.s.dt.on('stateSaveParams.dtsp', function (e,
                                                               settings,
                                                               data) {
                    // If the data being passed in is empty then state clear
                    // must have occured so clear the panes state as well
                    ...
                    var arrayFilter;
                    var collapsed;
                    // Get all of the data needed for the state save from the
                    // pane
            ...
            this.s.dtPane.on('user-select.dtsp', function (e,
                                                           _dt,
                                                           type,
                                                           cell,
                                                           originalEvent) {
            ...
            // When the button to order by the name of the options is clicked
            // then change the ordering to whatever it isn't currently
            this.dom.nameButton.off('click.dtsp');
            this.dom.nameButton.on('click.dtsp', function () {
                var currentOrder = _this.s.dtPane.order()[0][1];
                _this.s.dtPane.order([0, currentOrder === 'asc' ? 'desc' : 'asc']).draw();
                // This state save is required so that the ordering of the
                // panes is maintained
                _this.s.dt.state.save();
            });
            // When the button to order by the number of entries in the column
            // is clicked then change the ordering to whatever it isn't
            // currently
            ...
            this.dom.clear.on('click.dtsp', function () {
                ...
                searches.each(function () {
                    // set the value of the search box to be an empty string
                    // and then search on that, effectively reseting
                    $(this).val('');
                    $(this).trigger('input');
                });
                _this.clearPane();
            });
            // When the search button is clicked then draw focus to the search
            // box
            this.dom.searchButton.off('click.dtsp');
            this.dom.searchButton.on('click.dtsp', function () {
                _this.dom.searchBox.focus();
            });
            // When a character is inputted into the searchbox search the pane
            // for matching values. Doing it this way means that no button has
            // to be clicked to trigger a search, it is done asynchronously
            ...
            this.dom.searchBox.on('input.dtsp', function () {
                ...
                // This state save is required so that the searching on the
                // panes is maintained
                _this.s.dt.state.save();
            });
            return true;
        ...
        /**
         * Takes in potentially undetected rows and adds them to the array if
         * they are not yet featured
         *
         ...
         */
        SearchPane.prototype._addOption = function (filter,
                                                    display,
                                                    sort,
                                                    type,
                                                    arrayFilter,
                                                    bins) {
            // If the filter is an array then take a note of this, and add the
            // elements to the arrayFilter array
            if (Array.isArray(filter) || filter instanceof dataTable.Api) {
                ...
                // If the values were affected by othogonal data and are not an
            // array then check if it is already present
         ...
         * @last boolean to indicate whether this pane was the last one to have
         *     a selection made
         */
        SearchPane.prototype._buildPane = function (selectedRows,
            ...
            // If the listeners have not been set yet then using the latest
            // state may result in funny errors
            if (this.s.listSet) {
            ...
            if (this.colExists) {
                ...
                if (!this.s.dt.page.info().serverSide &&
                    ...
                    // Only run populatePane if the data has not been collected
                    // yet
                    ...
                    // Don't show the pane if there isn't enough variance in
                    // the data, or there is only 1 entry for that pane
                    ...
                    // the total count for the whole table to display alongside
                    // the displayed count
                    ...
                    // Don't show the pane if there isnt enough variance in the
                    // data, or there is only 1 entry for that pane
                    if (this.s.displayed === false &&
                        ((colOpts.show === undefined && colOpts.threshold === null ?
                                uniqueRatio > this.c.threshold :
                                uniqueRatio > colOpts.threshold) ||
            ...
            this._displayPane();
            if (!this.s.listSet) {
                // Here, when the state is loaded if the data object on the
                // original table is empty, then a state.clear() must have
                // occurred, so delete all of the panes tables state objects
                // too.
                this.dom.dtP.on('stateLoadParams.dt', function (e,
                                                                settings,
                                                                data) {
                ...
                columnDefs: [
                    {
                        ...
                        render: function (data, type, row) {
                            ...
                            // We are displaying the count in the same columne
                            // as the name of the search option. This is so
                            // that there is not need to call columns.adjust()
                        ...
                        targets: 0,
                        // Accessing the private datatables property to set
                        // type based on the original table. This is null if
                        // not defined by the user, meaning that automatic type
            ...
            // As the pane table is not in the document yet we must initialise
            // select ourselves eslint-disable-next-line no-extra-parens
            ...
            if (this.colExists) {
                ...
                for (var i = 0, ien = rowData.arrayFilter.length; i < ien; i++) {
                    ...
                    if (this.s.dt.page.info().serverSide &&
                        ...
                        // Just pass an empty string as the message will be
                        // calculated based on that in addRow()
            ...
            // If there are custom options set or it is a custom pane then get
            // them
                ...
                if (selection !== undefined) {
                    for (var _g = 0, _h = this.s.dtPane.rows().indexes().toArray(); _g < _h.length; _g++) {
                        ...
                        if (this.s.dtPane.row(row).data() !== undefined &&
                            ...
                            // If this is happening when serverSide processing
                            // is happening then different behaviour is needed
                            if (this.s.dt.page.info().serverSide) {
                                this.s.serverSelecting = true;
            ...
            // Reload the selection, searchbox entry and ordering from the
            // previous state Need to check here if SSP that this is the first
            // draw, otherwise it will infinite loop
            if (loadedFilter &&
                ...
                for (var _j = 0, _k = loadedFilter.searchPanes.panes; _j < _k.length; _j++) {
                    ...
                    if (pane.id === this.s.index) {
                        // Save some time by only triggering an input if there
                        // is a value
        ...
        /**
         * Update the array which holds the display and filter values for the
         * table
         */
        ...
        SearchPane.prototype._displayPane = function () {
            ...
            // If there are more than 3 columns defined then make there be a
            // smaller gap between the panes
            if (layVal > 3) {
            ...
            // If no selections have been made in the pane then disable the
            // clear button
            ...
            // Create the contents of the searchCont div. Worth noting that
            // this function will change when using semantic ui
            ...
            // If the count column is hidden then don't display the ordering
            // button for it
            if (this.c.viewCount &&
                colOpts.viewCount &&
                this.c.orderable &&
         ...
         *
         * @returns {object} The options for the row extended to include the
         *     options from the user.
         */
        SearchPane.prototype._getBonusOptions = function () {
            // We need to reset the thresholds as if they have a value in
            // colOpts then that value will be used
            var defaultMutator = {
                orthogonal: {
                    threshold: null
                },
         ...
         *
         * @returns {Array} Returns the array of rows which have been added to
         *     the pane
         */
        SearchPane.prototype._getComparisonRows = function () {
            var colOpts = this.s.colOpts;
            // Find the appropriate options depending on whether this is a pane
            // for a specific column or a custom pane
            ...
            var rows = [];
            // Clear all of the other rows from the pane, only custom options
            // are to be displayed when they are defined
            this.s.dtPane.clear();
            for (var _i = 0, options_1 = options; _i < options_1.length; _i++) {
                ...
                if (typeof comp.value === 'function') {
                    // Count the number of times the function evaluates to true
                    // for the data currently being displayed
                    ...
                    // Count the number of times the function evaluates to true
                    // for the original data in the Table
                ...
                // If cascadePanes is not active or if it is and the
                // comparisonObj should be shown then add it to the pane
        ...
        /**
         * This method allows for changes to the panes and table to be made
         * when a selection or a deselection occurs
         *
        ...
        /**
         * Fill the array with the values that are currently being displayed in
         * the table
         *
         * @param last boolean to indicate whether this was the last pane a
         *     selection was made in
         */
        SearchPane.prototype._populatePane = function (last) {
            ...
            this.s.rowData.bins = {};
            var settings = this.s.dt.settings()[0];
            // If cascadePanes or viewTotal are active it is necessary to get
            // the data which is currently being displayed for their
            // functionality. Also make sure that this was not the last pane to
            // have a selection made
            if (!this.s.dt.page.info().serverSide) {
         ...
         * @param arrayFilter The array that is to be populated with row
         *     Details
         * @param bins The bins object that is to be populated with the row
         *     counts
         */
        SearchPane.prototype._populatePaneArray = function (rowIdx,
            ...
            var colOpts = this.s.colOpts;
            // Retrieve the rendered data from the cell using the fnGetCellData
            // function rather than the cell().render API method for
            // optimisation
        ...
        SearchPane.prototype._reloadSelect = function (loadedFilter) {
            ...
            var idx;
            // For each pane, check that the loadedFilter list exists and is
            // not null, find the id of each search item and set it to be
            // selected.
            for (var i = 0; i < loadedFilter.searchPanes.panes.length; i++) {
                if (loadedFilter.searchPanes.panes[i].id === this.s.index) {
                    idx = i;
        ...
        /**
         * This method decides whether a row should contribute to the pane or
         * not
         *
        ...
        SearchPane.prototype._search = function (filter, dataIndex) {
            var colOpts = this.s.colOpts;
            var table = this.s.dt;
            // For each item selected in the pane, check if it is available in
            // the cell
            for (var _i = 0, _a = this.selections; _i < _a.length; _i++) {
                var colSelect = _a[_i];
                if (typeof colSelect.filter === 'string' && typeof filter === 'string') {
                    // The filter value will not have the &amp; in place but a
                    // &, so we need to do a replace to make sure that they
                    // will match
                ...
                if (Array.isArray(filter)) {
                    ...
                    // if the filter is a function then does it meet the
                    // criteria
                // of that function or not
                    ...
                    if (colSelect.filter.call(table, table.row(dataIndex).data(), dataIndex)) {
                        ...
                        // If the combiner is an "and" then we need to check
                        // against all possible selections
                        // so if it fails here then the and is not met and
                        // return
                    // false
                ...
                // otherwise if the two filter values are equal then return true
                    ...
                    // Loose type checking incase number type in column
                    // comparing to a string eslint-disable-next-line eqeqeq
                    !(typeof filter === 'string' && filter.length === 0) && filter == colSelect.filter ||
                    colSelect.filter === null && typeof filter === 'string' && filter === '') {
                    return true;
                }
            }
            // If the combiner is an and then we need to check against all
            // possible selections so return true here if so because it would
            // have returned false earlier if it had failed
            if (colOpts.combiner === 'and') {
                return true;
            }
                // Otherwise it hasn't matched with anything by this point so it
            // must be false
            else {
                return false;
            }
         ...
         *
         * NOTE This is overridden when semantic ui styling in order to
         * integrate the search button into the text box.
         */
        ...
        SearchPane.prototype._searchExtras = function () {
            ...
            // If a filter has been applied then outline the respective pane,
        ...
        /**
         * Finds the ratio of the number of different options in the table to
         * the number of rows
         *
         ...
         *
         * @param draw a flag to define whether this has been called due to a
         *     draw event or not
         */
        SearchPane.prototype._updateCommon = function (draw) {
            ...
            if (!this.s.dt.page.info().serverSide &&
                ...
                var rowData = this.s.rowData;
                // Clear the pane in preparation for adding the updated search
                // options
                this.s.dtPane.clear();
                // If it is not a custom pane
                if (this.colExists) {
                    ...
                    if (rowData.arrayFilter.length === 0) {
                        ...
                        // If cascadePanes is active and the table has returned
                        // to its default state then
                    // there is a need to update certain parts ofthe rowData.
                        ...
                        // Otherwise if viewTotal or cascadePanes is active then
                    // the data from the table must be read.
                    else if (this.c.viewTotal || this.c.cascadePanes) {
                        this._populatePane(!this.s.filteringActive);
                    }
                    // If the viewTotal option is selected then find the totals
                    // for the table
                    if (this.c.viewTotal) {
                        this._detailsPane();
                    } else {
                    ...
                    var _loop_1 = function (dataP) {
                        // If both view Total and cascadePanes have been
                        // selected and the count of the row is not 0 then add
                        // it to pane Do this also if the viewTotal option has
                        // been selected and cascadePanes has not
                        if (dataP &&
                            ...
                            // Find out if the filter was selected in the
                            // previous search, if so select it and remove from
                            // array.
                ...
                // Add search options which were previously selected but whos
                // results are no longer present in the resulting data set.
    ...
    var SearchPanes = /** @class */ (function () {
        function SearchPanes(paneSettings, opts, fromInit) {
            ...
            // We are using the xhr event to rebuild the panes if required due
            // to viewTotal being enabled If viewTotal is not enabled then we
            // simply update the data from the server
        ...
        SearchPanes.prototype.clearSelections = function () {
            ...
            // For each searchBox set the input text to be empty and then
            // trigger an input on them so that they no longer filter the panes
            searches.each(function () {
                $$1(this).val('');
                $$1(this).trigger('input');
            });
            var returnArray = [];
            // Clear the selectionList to prevent cascadePanes from reselecting
            // rows
        ...
        SearchPanes.prototype.rebuild = function (targetIdx, maintainSelection) {
            ...
            // As a rebuild from scratch is required, empty the searchpanes
            // container.
            var returnArray = [];
            // Rebuild each pane individually, if a specific pane has been
            // selected then only rebuild that one
            if (targetIdx === false) {
            ...
            for (var _i = 0, _a = this.s.panes; _i < _a.length; _i++) {
                ...
                pane.clearData();
                returnArray.push(
                    // Pass a boolean to say whether this is the last choice
                    // made for maintaining selections when rebuilding
            ...
            // If the selections are to be maintained, then it is safe to
            // assume that paging is also to be maintained Otherwise, the
            // paging should be reset
        ...
        SearchPanes.prototype.redrawPanes = function (rebuild) {
            ...
            var table = this.s.dt;
            // Only do this if the redraw isn't being triggered by the panes
            // updating themselves
            if (!this.s.updating && !this.s.dt.page.info().serverSide) {
                ...
                // If the number of rows currently visible is equal to the
                // number of rows in the table then there can't be any
                // filtering taking place
                if (selectTotal === 0 &&
                    table.rows({search: 'applied'}).data().toArray().length === table.rows().data().toArray().length) {
                    filterActive = false;
                }
                    // Otherwise if viewTotal is active then it is necessary to
                    // determine which panes a select is present in. If there
                    // is only one pane with a selection present then it should
                    // not show the filtered message as
                // more selections may be made in that pane.
                    ...
                    for (var _b = 0, _c = this.s.panes; _b < _c.length; _b++) {
                        ...
                        if (pane.s.dtPane !== undefined) {
                            ...
                            // If filterPane === -1 then a pane with a
                            // selection has not been found yet, so set
                            // filterPane to that panes index
                            if (selectLength > 0 && filterPane === -1) {
                                filterPane = pane.s.index;
                            }
                                // Then if another pane is found with a
                                // selection then set filterPane to null to
                    ...
                    // If the searchbox is in place and filtering is applied
                    // then need to cascade down anyway
                    if (selectTotal === 0) {
                        filterPane = null;
                    }
                }
                ...
                if (!this.regenerating) {
                    for (var _f = 0, _g = this.s.panes; _f < _g.length; _f++) {
                        var pane = _g[_f];
                        // Identify the pane where a selection or deselection
                        // has been made and add it to the list.
                    ...
                    // Remove selections from the list from the pane where a
                    // deselect has taken place
                    for (var i = 0; i < this.s.selectionList.length; i++) {
                        if (this.s.selectionList[i].index !== deselectIdx || this.s.selectionList[i].protect === true) {
                            var further = false;
                            // Find out if this selection is the last one in
                            // the list for that pane
                            for (var j = i + 1; j < this.s.selectionList.length; j++) {
                                if (this.s.selectionList[j].index === this.s.selectionList[i].index) {
                                    further = true;
                                }
                            }
                            // If there are no selections for this pane in the
                            // list then just push this one
                            if (!further) {
                    ...
                    // Update all of the panes to reflect the current state of
                    // the filters
                    ...
                    // If the length of the selections are different then some
                    // of them have been removed and a deselect has occured
                    if (newSelectionList.length > 0 && (newSelectionList.length < this.s.selectionList.length || rebuild)) {
                        ...
                        // Update all of the other panes as you would just
                        // making a normal selection
        ...
        SearchPanes.prototype.resizePanes = function () {
            if (this.c.layout === 'auto') {
                ...
                var target = Math.floor(contWidth / 260.0); // The neatest
                                                            // number of panes
                                                            // per row
                var highest = 1;
                var highestmod = 0;
                ...
                if (target === displayCount) {
                    ...
                    // Go from the target down and find the value with the most
                    // panes left over, this will be the best fit
                    for (var ppr = target; ppr > 1; ppr--) {
                        ...
                        if (rem === 0) {
                            ...
                            // If there are more left over at this amount of
                            // panes per row (ppr)
        ...
        SearchPanes.prototype._attach = function () {
            ...
            // Attach the container for each individual pane to the overall
            // container
            for (var _i = 0, _a = this.s.panes; _i < _a.length; _i++) {
                var pane = _a[_i];
        ...
        SearchPanes.prototype._attachMessage = function () {
            ...
            // If the message is an empty string then searchPanes.emptyPanes is
            // undefined, therefore the pane container should be removed from
            // the display
            if (message === null) {
        ...
        /**
         * Attaches the panes to the document and displays a message or hides
         * if there are none
         */
        SearchPanes.prototype._attachPaneContainer = function () {
            ...
            // Otherwise attach the custom message or remove the container from
            // the display
            return this._attachMessage();
        };
        /**
         * Prepares the panes for selections to be made when cascade is active
         * and a deselect has occured
         *
        ...
        SearchPanes.prototype._cascadeRegen = function (newSelectionList,
                                                        selectTotal) {
            // Set this to true so that the actions taken do not cause this to
            ...
            // Let the pane know that a cascadeRegen is taking place to avoid
            // unexpected behaviour and clear all of the previous selections in
            // the pane
            for (var _i = 0, _a = this.s.panes; _i < _a.length; _i++) {
                var pane = _a[_i];
                pane.setCascadeRegen(true);
                pane.setClear(true);
                // If this is the same as the pane with the only selection then
                // pass it as a parameter into clearPane
            ...
            // Rebin panes
            this.s.dt.draw();
            // While all of the selections have been removed, check the table
            // lengths If they are different, another filter is in place and we
            // need to force viewTotal to be used
            ...
            // Set the selection list property to be the list without the
            // selections from the deselect pane
            ...
            this.regenerating = false;
            // ViewTotal has already been forced at this point so can cancel
            // that for future
            if (tableLength !== noSelectionsTableLength) {
                for (var _h = 0, _j = this.s.panes; _h < _j.length; _h++) {
                    var pane = _j[_h];
        ...
        SearchPanes.prototype._checkMessage = function () {
            ...
            for (var _i = 0, _a = this.s.panes; _i < _a.length; _i++) {
                var pane = _a[_i];
                if (pane.s.displayed === true) {
                    // Ensure that the empty message is removed if a pane is
                    // displayed
                    this.dom.emptyMessage.remove();
                    this.dom.titleRow.removeClass(this.classes.hide);
                    return;
        ...
        /**
         * Checks which panes are collapsed and then performs relevant actions
         * to the collapse/show all buttons
         *
        ...
        SearchPanes.prototype._checkCollapse = function () {
            ...
            // If this flag is still true, no panes are open so the close
            // button should be disabled
            if (disableClose) {
                this.dom.collapseAll.addClass(this.classes.disabledButton).attr('disabled', 'true');
            }
            // If this flag is still true, no panes are closed so the show
            // button should be disabled
            if (disableShow) {
        ...
        /**
         * Gets the selection list from the previous state and stores it in the
         ...
         *
         * @param newSelectionList the list of selections to be made, in the
         *     order they were originally selected
         */
            ...
            for (var i = 0; i < newSelectionList.length; i++) {
                var _loop_1 = function (pane) {
                    if (pane.s.index === newSelectionList[i].index && pane.s.dtPane !== undefined) {
                        // When regenerating the cascade selections we need
                        // this flag so that the panes are only ignored if it
                        // is the last selection and the pane for that
                        // selection
                        if (i === newSelectionList.length - 1) {
                            pane.s.lastCascade = true;
                        }
                        // if there are any selections currently in the pane
                        // then deselect them as we are about to make our new
                        // selections
                        ...
                        // select every row in the pane that was selected
                        // previously
                ...
                // which pane has the index of the selection. This is also
                // important for colreorder etc
                for (var _i = 0, _a = this.s.panes; _i < _a.length; _i++) {
                    var pane = _a[_i];
                    _loop_1(pane);
                }
        ...
        /**
         * Declares the instances of individual searchpanes dependant on the
         * number of columns. It is necessary to run this once preInit has
         * completed otherwise no panes will be created as the column count
         * will be 0.
         *
         ...
         */
        SearchPanes.prototype._paneDeclare = function (table,
                                                       paneSettings,
                                                       opts) {
            var _this = this;
            // Create Panes
            ...
            // If there is any extra custom panes defined then create panes for
            // them too
            var rowLength = table.columns().eq(0).toArray().length;
            ...
            // If this internal property is true then the DataTable has been
            // initialised already
            if (this.s.dt.settings()[0]._bInitComplete) {
                this._startup(table);
            } else {
                // Otherwise add the paneStartup function to the list of
                // functions that are to be run when the table is initialised.
                // This will garauntee that the panes are initialised before
                // the init event and init Complete callback is fired
        ...
        /**
         * Works out which panes to update when data is recieved from the
         * server and viewTotal is active
         */
        SearchPanes.prototype._serverTotals = function () {
            ...
            for (var _i = 0, _a = this.s.panes; _i < _a.length; _i++) {
                var pane = _a[_i];
                // Identify the pane where a selection or deselection has been
                // made and add it to the list.
                if (pane.s.selectPresent) {
                    this.s.selectionList.push({
                        index: pane.s.index,
                        protect: false,
            ...
            // Build an updated list based on any selections or deselections
            // added
            if (!selectPresent) {
                this.s.selectionList = [];
            } else {
                ...
                for (var i = 0; i < this.s.selectionList.length; i++) {
                    var further = false;
                    // Find out if this selection is the last one in the list
                    // for that pane
                    for (var j = i + 1; j < this.s.selectionList.length; j++) {
                        if (this.s.selectionList[j].index === this.s.selectionList[i].index) {
                            further = true;
                    ...
                    // If there are no selections for this pane in the list
                    // then just push this one
                    if (!further) {
                        var push = false;
                        for (var _b = 0, _c = this.s.panes; _b < _c.length; _b++) {
                            var pane = _c[_b];
            ...
            var initIdx = -1;
            // If there has been a deselect and only one pane has a selection
            ...
            if (deselectPresent && this.s.selectionList.length === 1) {
                ...
                // Otherwise if there are more 1 selections then find the last
                // one
            // and set it to not update that pane
                ...
                // Otherwise if there are no selections then find where that
                // took
            // place and do not update to maintain scrolling
            else if (this.s.selectionList.length === 0) {
                for (var _h = 0, _j = this.s.panes; _h < _j.length; _h++) {
                    var pane = _j[_h];
         ...
         * Also sets and performs checks on current panes to see if they are
         * collapsed
         */
        SearchPanes.prototype._setCollapseListener = function () {
            ...
            for (var _i = 0, _a = this.s.panes; _i < _a.length; _i++) {
                var pane = _a[_i];
                // We want to make the same check whenever there is a
                // collapse/expand
        ...
        /**
         * Initialises the tables previous/preset selections and initialises
         * callbacks for events
         *
         * @param table the parent table for which the searchPanes are being
         *     created
         */
        SearchPanes.prototype._startup = function (table) {
            ...
            // If the layout is set to auto then the panes need to be resized
            // to their best fit
            if (this.c.layout === 'auto') {
                this.resizePanes();
            }
            ...
            this._checkMessage();
            // When a draw is called on the DataTable, update all of the panes
            // incase the data in the DataTable has changed
            ...
            // Whenever a state save occurs store the selection list in the
            // state object
            ...
            if (this.s.dt.page.info().serverSide) {
                ...
                table.on('preXhr.dt', function (e, settings, data) {
                    ...
                    // If there is a filter to be applied, then we need to read
                    // from the start of the result set and set the paging to
                    // 0. This matches the behaviour of client side processing
                    if (filterCount > 0) {
                        // If the number of filters has changed we need to read
                        // from the start of the result set and reset the
                        // paging
                        if (filterCount !== _this.s.filterCount) {
                            data.start = 0;
                            _this.s.page = 0;
                        }
                            // Otherwise it is a paging request and we need to
                            // read
                        // from whatever the paging has been set to
                        else {
            ...
            // If the data is reloaded from the server then it is possible that
            // it has changed completely, so we need to rebuild the panes
            this.s.dt.on('xhr', function (e, settings, json, xhr) {
                ...
                if (!_this.s.dt.page.info().serverSide) {
                    _this.s.dt.one('preDraw', function () {
                        ...
                        for (var _i = 0, _a = _this.s.panes; _i < _a.length; _i++) {
                            var pane = _a[_i];
                            pane.clearData(); // Clears all of the bins and
                                              // will mean that the data has to
                                              // be re-read
                            // Pass a boolean to say whether this is the last
                            // choice made for maintaining selections when
                            // rebuilding
            ...
            // PreSelect any selections which have been defined using the
            // preSelect option
            for (var _h = 0, _j = this.s.panes; _h < _j.length; _h++) {
                var pane = _j[_h];
                if (pane !== undefined &&
            ...
            // If cascadePanes is active then make the previous selections in
            // the order they were previously
            ...
            this._updateFilterCount();
            // If the table is destroyed and restarted then clear the
            // selections so that they do not persist.
            ...
            // When the clear All button has been pressed clear all of the
            // selections in the panes
            if (this.c.clear) {
                this.dom.clearAll.on('click.dtsps', function () {
                    _this.clearSelections();
                });
            ...
            table.settings()[0]._searchPanes = this;
            // This state save is required so that state is maintained over
            // multiple refreshes if no actions are made
        ...
        SearchPanes.prototype._prepViewTotal = function (selectTotal) {
            ...
            for (var _i = 0, _a = this.s.panes; _i < _a.length; _i++) {
                ...
                if (pane.s.dtPane !== undefined) {
                    var selectLength = pane.s.dtPane.rows({selected: true}).data().toArray().length;
                    // If filterPane === -1 then a pane with a selection has
                    // not been found yet, so set filterPane to that panes
                    // index
                    if (selectLength > 0 && filterPane === -1) {
                        filterPane = pane.s.index;
                        filterActive = true;
                    }
                        // Then if another pane is found with a selection then
                        // set filterPane to null to
                    // show that multiple panes have selections present
                    else if (selectLength > 0) {
                        filterPane = null;
                    }
            ...
            // Update all of the panes to reflect the current state of the
            // filters
            for (var _b = 0, _c = this.s.panes; _b < _c.length; _b++) {
                var pane = _c[_b];
                if (pane.s.dtPane !== undefined) {
        ...
        SearchPanes.prototype._updateFilterCount = function () {
            ...
            // Run the message through the internationalisation method to
            // improve readability
